...
def todos(request):
    if request.method == 'POST':
        ...
        return render(request, 'todolist/todos.html', {'form': form, 'tasks': tasks, 'user': user})
    else:
        if 'username' not in request.COOKIES:
            ...
            username.save()
            response = redirect(reverse('todolist:todos'))
            # 604800s = 1 week
...
def delete(request, id):
    if 'username' in request.COOKIES and check_user_validity(request):
        ...
        return redirect(reverse('todolist:todos'))
    else:
